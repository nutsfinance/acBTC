...
contract CurveRenCrvMigrator is IMigrator {
    ...
    address public constant exchange = address(0); // To be added after ACoconutExchange is deployed
    address public constant vault = address(0);    // To be added after ACoconutVault is deployed
    ...
    function migrate() public override {
        ...
        if (_wbtc > 0 || _renbtc > 0) {
            ...
            amounts[0] = _wbtc;
            amounts[1] = _renbtc;
            ACoconutExchange(exchange).mint(amounts, 0);
        }
        uint256 _get = IERC20(get).balanceOf(address(this));
        if (_get > 0) {
            IERC20(get).safeTransfer(vault, _get);
        }
    }
}